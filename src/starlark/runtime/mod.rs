...
pub fn evaluate_file(path: &Path) -> anyhow::Result<Vec<PackageEntry>> {
...
pub fn execute_function(path: &Path, function_name: &str, argument: &str) -> anyhow::Result<()> {
    let filename = path.to_string_lossy().into_owned();
    let content = fs::read_to_string(path)
        .with_context(|| format!("Failed to read file: {}", path.display()))?;
    ...
    setup_context(&module, format!("{}:exec", filename));
...
fn setup_context(module: &Module, filename: String) {
